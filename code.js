...
// Helper function to convert PascalCase or camelCase name to kebab-case for CSS classes
function toKebabCase(name) {
    if (!name)
        return ''; // Handle empty names
    return name
        .replace(/[^a-zA-Z0-9\s_-]/g, '') // Remove invalid chars first
        .replace(/([a-z0-9])([A-Z])/g, '$1-$2') // Add hyphen before uppercase letters preceded by lowercase/number
        .replace(/([A-Z])([A-Z][a-z])/g, '$1-$2') // Add hyphen between consecutive uppercase letters if followed by lowercase
...
function mapFigmaTypeToJsonType(node) {
    ...
    switch (node.type) {
        ...
        case 'VECTOR': // Treat vectors as icons or images
            if (node.name.toLowerCase().includes('icon'))
                return 'Icon';
            return 'Image'; // Better default for vectors than Container
        case 'TEXT':
...
function inferTag(node) {
    ...
    switch (node.type) {
        ...
        case 'VECTOR': // Better handling for vectors
            if (node.name.toLowerCase().includes('icon'))
                return 'span'; // span with background-image is simpler than SVG
            return 'img'; // img tag is more appropriate than svg for placeholder
        case 'ELLIPSE':
        case 'POLYGON':
        case 'STAR':
        case 'LINE':
...
function extractLayoutStyles(node, styles) {
    ...
    if ('x' in node && 'y' in node && node.parent && 'layoutMode' in node.parent && node.parent.layoutMode === 'NONE') {
        ...
        if ('constraints' in node && node.constraints && node.constraints.horizontal !== 'CENTER' && node.constraints.vertical !== 'CENTER') {
            ...
            // Add warning comment about absolute positioning
            styles._comment = 'Using absolute positioning. Consider using Auto Layout in Figma for better responsive layouts.';
            // Log warning to console
            console.log(`Warning: ${node.name} uses absolute positioning. Consider using Auto Layout in Figma for better responsive layouts.`);
        }
    }
}
...
function extractFillStyles(node, styles) {
    // Don't apply background color to vectors - they should be handled differently
    if (node.type === 'VECTOR') {
        // For vectors, we might want to store color differently or not at all for V1
        return;
    }
...
function extractTextStyles(node, styles) {
    if (node.type === 'TEXT') {
        ...
        if (textNode.fontName && textNode.fontName !== figma.mixed) {
            ...
            // Map Figma font styles to standard CSS values
            if (textNode.fontName.style) {
                ...
                if (style.includes('italic')) {
                    ...
                    // Default to normal if not explicitly italic (covers "Regular", "Medium", etc.)
                    styles.fontStyle = 'normal';
                }
                // Optionally handle oblique if needed
            }
        }
        // Handle lineHeight
