...
figma.ui.onmessage = async (msg) => { // Make the handler async
  if (msg.type === 'generate-json') {
    await processSelection(); // Await the async processing
  }
};

async function processSelection() { // Make this async
...
// --- SVG Export Helper ---
async function getSvgData(node: SceneNode): Promise<string | null> {
    try {
...
async function processNode(node: SceneNode): Promise<any | null> {
    ...
    const jsonNode: any = {
        ...
        tag: svgString ? 'svg' : inferTag(node), // Use 'svg' tag if we have SVG data
        styles: {},
        attributes: {},
    ...
    if (Object.keys(jsonNode.styles).length === 0) delete jsonNode.styles;
    if (Object.keys(jsonNode.attributes).length === 0) delete jsonNode.attributes;
    if (!jsonNode.content) delete jsonNode.content;
...
function mapFigmaTypeToJsonType(node: SceneNode): string {
    ...
    switch (node.type) {
        ...
        case 'RECTANGLE':
             ...
             if (node.name.toLowerCase().includes('input')) return 'Input'; // Check before Container
             if (node.name.toLowerCase().includes('image')) return 'Image';
             return 'Container';
        case 'TEXT':
            if (node.name.toLowerCase().includes('link')) return 'Link';
            return 'Text';
        default:
...
function inferTag(node: SceneNode): string {
     ...
     switch (node.type) { // Use original Figma type for tag inference where appropriate
        ...
        case 'RECTANGLE':
            ...
            if (node.name.toLowerCase().includes('form')) return 'form';
            if (node.name.toLowerCase().includes('fieldset')) return 'fieldset';
            return 'div';
        case 'TEXT':
            const fontSize = (node as TextNode).fontSize;
            const fontWeight = (node as TextNode).fontWeight;
            if (typeof fontSize === 'number' && fontSize > 20 && typeof fontWeight === 'number' && fontWeight >= 600) return 'h2';
            if (typeof fontSize === 'number' && fontSize > 17 && typeof fontWeight === 'number' && fontWeight >= 600) return 'h3';
            if (node.name.toLowerCase().includes('label')) return 'label';
            if (node.name.toLowerCase().includes('legend')) return 'legend';
            if (node.name.toLowerCase().includes('link')) return 'a';
            if (node.name.toLowerCase().includes('paragraph') || node.characters.length > 100) return 'p';
            return 'span';
...
function extractSizeStyles(node: SceneNode, styles: any) {
    if ('width' in node) styles.width = `${round(node.width)}px`;
    if ('height' in node) styles.height = `${round(node.height)}px`;
}

function extractLayoutStyles(node: SceneNode, styles: any) {
    if ('layoutMode' in node && node.layoutMode !== 'NONE') {
        styles.display = 'flex';
        styles.flexDirection = node.layoutMode === 'VERTICAL' ? 'column' : 'row';
        if (node.primaryAxisAlignItems) styles.justifyContent = mapAlignItems(node.primaryAxisAlignItems);
        if (node.counterAxisAlignItems) styles.alignItems = mapAlignItems(node.counterAxisAlignItems);
        if (node.itemSpacing && node.itemSpacing > 0) styles.gap = `${round(node.itemSpacing)}px`;
    }

    const padding: Record<string, number> = {};
    if ('paddingTop' in node && node.paddingTop > 0) padding.top = round(node.paddingTop);
    if ('paddingBottom' in node && node.paddingBottom > 0) padding.bottom = round(node.paddingBottom);
    if ('paddingLeft' in node && node.paddingLeft > 0) padding.left = round(node.paddingLeft);
    if ('paddingRight' in node && node.paddingRight > 0) padding.right = round(node.paddingRight);
    if (Object.keys(padding).length > 0) styles.padding = formatSpacing(padding);
...
function formatSpacing(padding: {top?: number, bottom?: number, left?: number, right?: number}): string {
    const top = padding.top ?? 0;
    const bottom = padding.bottom ?? 0;
    const left = padding.left ?? 0;
    const right = padding.right ?? 0;
    if (top === bottom && left === right && top === left) return `${top}px`;
    if (top === bottom && left === right) return `${top}px ${left}px`;
    if (left === right) return `${top}px ${left}px ${bottom}px`;
    return `${top}px ${right}px ${bottom}px ${left}px`;
}


function extractFillStyles(node: SceneNode, styles: any) {
...
function extractStrokeStyles(node: SceneNode, styles: any) {
...
function extractEffectStyles(node: SceneNode, styles: any) {
...
function extractCornerRadiusStyles(node: SceneNode, styles: any) {
    if ('cornerRadius' in node) {
        if (typeof node.cornerRadius === 'number' && node.cornerRadius > 0) {
            styles.borderRadius = `${round(node.cornerRadius)}px`;
             ...
             const tl = round(node.topLeftRadius ?? 0);
             const tr = round(node.topRightRadius ?? 0);
             const bl = round(node.bottomLeftRadius ?? 0);
             const br = round(node.bottomRightRadius ?? 0);
...
function extractTextStyles(node: SceneNode, styles: any) {
    if (node.type === 'TEXT') {
        if (node.fontSize && typeof node.fontSize === 'number') styles.fontSize = `${round(node.fontSize)}px`;
        if (node.fontWeight && typeof node.fontWeight === 'number') styles.fontWeight = node.fontWeight.toString();
        if (node.fontName && typeof node.fontName !== 'symbol') styles.fontFamily = node.fontName.family;
        ...
        if (node.lineHeight && typeof node.lineHeight !== 'symbol' && node.lineHeight.unit !== 'AUTO') {
             styles.lineHeight = node.lineHeight.unit === 'PIXELS' ? `${round(node.lineHeight.value)}px` : `${round(node.lineHeight.value / (node.fontSize as number) * 100) / 100}`; // Unitless based on font size
        }
        if (node.textAlignHorizontal) styles.textAlign = node.textAlignHorizontal.toLowerCase();

        if (node.textDecoration) {
            if (node.textDecoration === 'UNDERLINE') styles.textDecoration = 'underline';
            if (node.textDecoration === 'STRIKETHROUGH') styles.textDecoration = 'line-through';
        }
...
function formatColor(color: RGB | RGBA, opacity?: number): string {
  const r = Math.round(color.r * 255);
  const g = Math.round(color.g * 255);
  const b = Math.round(color.b * 255);
  const a = opacity ?? ('a' in color ? color.a : 1);
  if (a === 1) {
